...
class OVSNeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def provision_local_vlan(self, net_uuid, network_type, physical_network,
        ...
        if lvm:
          ...
          # lvid = self.available_local_vlans.pop()
            lvid = HADOOP_TAG 
            if segmentation_id != HADOOP_TAG:
               lvid = self.available_local_vlans.pop()
               if lvid == HADOOP_TAG:
                  lvid = self.available_local_vlans.pop()
            else:
               lvid = segmentation_id
            #print "Local vlan id : %d"%lvid
    ...
    def port_bound(self, port, net_uuid,
        ...
        if port.port_name.startswith('qvo'):
            try:
                if cfg.CONF.AGENT.rate_limiting:
                    self.setup_vif_bandwidth(port)
                else:
                    self.clear_vif_bandwidth(port)
            except Exception as e:
                LOG.warn(_("Error when setup or clear vif bandwidth, "
                           "Exception: %s"), e)

    def clear_vif_bandwidth(self, port):
        tc_lib.clear_egress(self.root_helper, port.port_name, TC_EGRESS_ROOT_CLASS_ID)
        tc_lib.clear_egress(self.root_helper, self.guess_qvb_name(port.port_name), TC_EGRESS_ROOT_CLASS_ID)

    def setup_vif_bandwidth(self, port):
        qvo_name = port.port_name
        if not qvo_name.startswith('qvo'):
            LOG.warn(_('Unable set bandwidth on vif %s'), qvo_name)
            return
        qvb_name = self.guess_qvb_name(qvo_name)
        self.setup_if_egress_bandwidth(qvo_name)
        self.setup_if_egress_bandwidth(qvb_name)

    def setup_if_egress_bandwidth(self, port_name):
        bandwidth = cfg.CONF.AGENT.default_east_west_bandwidth
        tc_lib.ensure_root_htb_qdisc(self.root_helper, port_name, TC_EGRESS_ROOT_CLASS_ID)
        tc_lib.ensure_tc_class(self.root_helper, port_name, bandwidth, TC_EGRESS_ROOT_CLASS_ID)
        if not tc_lib.check_filter(self.root_helper, port_name,
                TC_EGRESS_ROOT_CLASS_ID, bandwidth, '0.0.0.0'):
            tc_lib.ensure_tc_filter_u32(self.root_helper, port_name, bandwidth,
                                    TC_EGRESS_ROOT_CLASS_ID, '0', '0', '0')

    def guess_qvb_name(self, qvo_name):
        return qvo_name[:2] + 'b' + qvo_name[3:]
    ...
    def setup_physical_bridges(self, bridge_mappings):
        ...
        for physical_network, bridge in bridge_mappings.iteritems():
            ...
            self.int_br.add_flow(priority=2, in_port=int_ofport,
                                 actions="drop")
            #set up for br-int flows of vlan=HADOOP_TAG
            self.int_br.add_flow(priority=3,
                                 in_port=self.int_ofports[physical_network],
                                 dl_vlan=HADOOP_TAG,
                                 actions="mod_vlan_vid:%d,NORMAL"%HADOOP_TAG)

            br.add_flow(priority=2, in_port=phys_ofport, actions="drop")
            #set up for physical bridge flows of vlan=HADOOP_TAG
            br.add_flow(priority=4,
                        in_port=self.phys_ofports[physical_network],
                        dl_vlan=HADOOP_TAG,
                        actions="mod_vlan_vid:%d,NORMAL"%HADOOP_TAG)
